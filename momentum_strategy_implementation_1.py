...
class Momentum(bt.Indicator):
    ...
    # except for the first 90 trading days
    ...
    def next(self):
        returns = np.log(self.data.get(self.params.period))
...
class MomentumStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        # SMA for SP500 index - we open long postitions when the SP500 index
        # is above it's SMA 
    ...
    def next(self):
        # a week has passed so we make trades if needed
        if self.counter % 5 == 0:
    ...
    def rebalance_portfolio(self):
        ...
        for index, single_stock in enumerate(self.sorted_data):
            # we can check whether are there open positons
            if self.getposition(self.data).size:
                # if the stock is not in the top 20% then close the long position
                # close the long position if the price falls below its 100 days MA
        ...
        if self.spy < self.sma200:
            return
        # consider the top 20% of te stocks and buy accordingly
...
if __name__ == '__main__':
    ...
    with open("algorithmic_trading_folder\\Data\\companies") as file_in:
        for line in file_in:
            stocks.append(line.strip('\n'))
            try:
                df = pd.read_csv("algorithmic_trading_folder\\Data\\" + line.strip('\n'), parse_dates=True, index_col=0)
                if len(df) > 100:
    ...
    # This is how we attach the strategy we have implemented to Cerebro
    ...
    print('Initial capital: $%2.f' % cerebro.broker.getvalue())
    results = cerebro.run()

    print(f"Sharpe: {results[0].analyzers.sharperatio.get_analysis()['sharperatio']:.3f}")
    print(f"Norm. Annual Return: {results[0].analyzers.sharperatio.get_analysis()['rnorm100']:.3f}")
    print(f"Max Drawdown: {results[0].analyzers.sharperatio.get_analysis()['drawdown']:.3f}")
    print(f"Capital: $2.f" % cerebro.broker.getvalue())
